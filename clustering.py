...
def test_params(**params):
    ...
    for t in thresholds:
        ...
        data["pred"] = pd.Series(y_pred)
        data.update({"t": t,"dataset": params["dataset"], "model": params["model"], "sub_model" : params["sub_model"]})
        # data.to_csv("data/pred_formatted_time_JT_INA.tsv", sep="\t", index=False)
        data.to_csv("data/recup_pred_medias.tsv", sep="\t", index=False)
