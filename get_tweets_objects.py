...
def format_tweet(row):
    tweet_dict = {}
    tweet_dict["text"] = row["full_text"]
    tweet_dict["id"] = row["id_str"]
    tweet_dict["created_at"] = row["created_at"]
...
def main():
    ...
    labeled_data = pd.read_csv(path, sep="\t", header=None, names=["label", "id"]).drop_duplicates()
    ...
    complete_data["label"] = complete_data["label"].astype(str).str.replace(".0", "").str.replace("nan", "")
    complete_data["text"] = complete_data["text"].str.replace("\t", " ").str.replace("\n", " ").str.replace("\r", " ")
    complete_data.to_csv("data/event2012_test.tsv", sep="\t", index=False)
